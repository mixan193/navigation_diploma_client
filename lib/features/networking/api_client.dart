...
class ApiClient {
  ...
  Future<Map<String, dynamic>> uploadScan(ScanUpload scan, { String? token }) async {
    final opts = Options(
      headers: token != null ? {'Authorization': 'Bearer $token'} : null,
    );
    final resp = await _dio.post('/v1/upload', data: scan.toJson(), options: opts);
    return resp.data;
  }
  ...
  Future<Map<String, dynamic>?> uploadScanAndGetPosition(ScanUpload scan, { String? token }) async {
    try {
      ...
      throw Exception('Ошибка отправки скана: ${e.response?.data ?? e.message}');
    }
  }
...
Future<List<AccessPointOut>> getAccessPoints(int buildingId) async {
  ...
  final aps = (resp.data as List).map((item) => AccessPointOut.fromJson(item)).toList();
  return aps;
}

// Обновление координат AP
Future<void> updateAccessPoint(String bssid, {required double x, required double y, required double z}) async {
  await _dio.put('/v1/ap/$bssid', data: {
    'x': x,
    'y': y,
    'z': z,
  });
